...
class CANPlotter:
    ...
    def decode_8byte_aligned(self, data):
        ...
        while i < len(data):
            ...
            if data[i:i+3] == b'\x11\x00\x01' or data[i:i+3] == b'\x11\x01\x01':
                ...
                timestamp = struct.unpack("<I", record[4:8])[0] + self.shift_bit * 20
    ...
    def update(self, frame):
        ...
        if data:
            ...
            for idx, bit in enumerate(self.bit_data):
                ...
                if actual_idx in frame_labels:
                    ...
                    if self.current_bits != []:
                        bit_string = ''.join(str(b) for b in self.current_bits)
                        decoded_bit = self.bits_to_hex(bit_string)
                        if len(self.current_bits) == 1:
                            new_x = (last_actual_idx * 20) + 12
                            rot = 90          
                            fz = 8
                        else: 
                            new_x = ((last_actual_idx + (len(self.current_bits) + stf_cnt_ali_t) / 2 )  * 20)
                            rot = 0
                            fz = 10
                        self.ax.text(new_x, -0.45, label, rotation=rot, fontsize=fz, ha='center', va='bottom', color='black')
                        self.ax.text(new_x, -0.4, decoded_bit, rotation=rot, fontsize=fz, ha='center', va='bottom', color='black')
            ...
            # self.state_data.insert(0,1)
            # self.state_data.insert(0,1)
            # self.timestamp_data.insert(0,100)
            # self.timestamp_data.insert(0,0)
